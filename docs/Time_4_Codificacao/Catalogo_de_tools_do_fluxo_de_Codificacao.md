...
| Arquivo | Conteúdo |
|---|---|
| `filesystem_coding.py` | `tool_criar_arquivo`, `tool_ler_arquivo`, `tool_substituir_trecho`, `tool_remover_arquivo`, `tool_salvar_relatorio` |
...
| `tool_remover_arquivo` (issue #388) | `adk/shared/tools/coding_tools/filesystem_coding.py` | `caminho: str`, `base_dir` (bound a `workspace_output/coder/src/` — invisível ao LLM) | `dict {sucesso, caminho, tipo, codigo, erro}` — `tipo` é `None` em **toda** falha | Remove um arquivo, ou uma pasta **vazia**, do workspace do coder — renomeação/movimentação, módulo descontinuado, arquivo criado por engano. Falha sempre de forma controlada (nunca levanta exceção, nem para entrada de tipo errado), com `codigo` explicando a recusa: `CAMINHO_INVALIDO` (tipo errado, absoluto ou `..` — recusados **com ou sem** `base_dir`), `CAMINHO_VAZIO`, `DIRETORIO_PROTEGIDO` (o próprio diretório sensível **ou** qualquer coisa dentro dele), `RAIZ_DO_WORKSPACE`, `FORA_DO_WORKSPACE` (o diretório-pai resolvido cai fora da raiz — pega **pai que é link simbólico**, que o `unlink()` seguiria), `CAMINHO_INEXISTENTE`, `DIRETORIO_NAO_VAZIO`, `PERMISSAO_NEGADA`, `ERRO_INESPERADO`. **Não é recursiva por decisão de projeto** — remoção em massa apagaria em silêncio artefato que ninguém revisou. Diferente da escrita, **não** aplica a whitelist de extensões: limpar o workspace não deve depender do que se pode criar. |
...
### Callbacks do coder (não são tools, não são chamados pelo LLM)

| Callback | Quando roda | Retorno | Descrição |
|---|---|---|---|
| `bloquear_sobrescrita_herdada` (`before_tool_callback`) | Antes de cada tool do coder | `dict` de erro (substitui a execução) ou `None` | Impede `tool_criar_arquivo` de sobrescrever integralmente arquivo **herdado de task anterior** (`SOBRESCRITA_INTER_TASK_BLOQUEADA`). Fotografia em `state["coder_arquivos_herdados_da_task"]`, tirada pelo `TaskIterator` e imutável durante os retries da task. |
| `auditar_remocao` (`after_tool_callback`, issue #388) | Depois de cada tool do coder | sempre `None` (não altera o retorno visto pelo LLM) | Só age sobre `tool_remover_arquivo`. (1) **Auditoria:** loga `[CODER][<task_id>] REMOVIDO ...` no sucesso e `[CODER][<task_id>] remoção RECUSADA ... <codigo>` na recusa — é o que amarra a remoção à task. (2) **Baseline:** remoção bem-sucedida tira o caminho (e o que estava sob ele, no caso de pasta) da fotografia de herdados, para que a task possa recriar o arquivo sem esbarrar no guard — remover é ato explícito, não sobrescrita cega. Recusa não altera a fotografia. |

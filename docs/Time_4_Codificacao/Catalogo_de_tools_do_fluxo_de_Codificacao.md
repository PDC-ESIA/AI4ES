...
| `tool_remover_arquivo` (issue #388) | `adk/shared/tools/coding_tools/filesystem_coding.py` | `caminho: str`, `base_dir` (bound a `workspace_output/coder/src/` — invisível ao LLM) | `dict {sucesso, caminho, tipo, codigo, erro}` | Remove um arquivo, ou uma pasta **vazia**, do workspace do coder — renomeação/movimentação, módulo descontinuado, arquivo criado por engano. Falha sempre de forma controlada (nunca levanta exceção), com `codigo` explicando a recusa: `CAMINHO_VAZIO`, `CAMINHO_INVALIDO` (absoluto ou `..`), `DIRETORIO_PROTEGIDO` (o próprio diretório sensível **ou** qualquer coisa dentro dele), `RAIZ_DO_WORKSPACE`, `CAMINHO_INEXISTENTE`, `DIRETORIO_NAO_VAZIO`, `PERMISSAO_NEGADA`, `ERRO_INESPERADO`. **Não é recursiva por decisão de projeto** — remoção em massa apagaria em silêncio artefato que ninguém revisou. Diferente da escrita, **não** aplica a whitelist de extensões: limpar o workspace não deve depender do que se pode criar. |

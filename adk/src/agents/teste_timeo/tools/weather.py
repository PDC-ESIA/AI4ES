...
class Weather(BaseModel):
    city: str = Field(default="", description="Cidade")
    temperature: str = Field(default="", description="Temperatura", max_length=1000)
    description: str = Field(
        default="", description="Descrição do tempo", max_length=1000
    )
...
def tool_get_weather(params: Weather, tool_context: ToolContext) -> dict:
    ...
    if isinstance(params, dict):
        try:
            ...
            return {
                "status": "Erro",
                "error": f"Dados de entrada inválidos: {e}",
                "result": None,
            }

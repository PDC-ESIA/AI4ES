...
class RelatorioSchema(BaseModel):
    conteudo: str = Field(
        ..., description="Conteúdo em Markdown do relatório de revisão"
    )
    nome_arquivo: str = Field(
        default="doubt_artifact_revisao.md", description="Nome do arquivo de saída"
    )
    ...
    def validar_extensao(cls, v):
        ...
        if not v.endswith(".md"):
            raise ValueError(
                "O relatório de revisão DEVE obrigatoriamente ser um arquivo Markdown (.md)"
            )
        return v
...
def tool_ler_diff(branch_alvo: str = "main") -> dict:
    ...
    resposta = subprocess.run(comando, capture_output=True, text=True, encoding="utf-8")

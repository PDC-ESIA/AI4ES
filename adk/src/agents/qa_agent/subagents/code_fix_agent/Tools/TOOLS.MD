...
## Por que a tool de parsing não está sendo usada no fluxo atual

No fluxo atual do subagente, a entrada principal já é o texto bruto do log/traceback. Nesse cenário, chamar primeiro a tool de parsing e só depois pedir para o agente interpretar o resultado parseado traz pouco ganho prático e adiciona custo operacional.

### Motivo principal

Processo em duas etapas:
- Etapa 1: parsear o texto bruto para uma estrutura intermediária.
- Etapa 2: fazer o agente interpretar essa estrutura para montar o diagnóstico/prompt.

Quando o agente já consegue interpretar diretamente o traceback bruto, a etapa intermediária vira redundante.

### Impactos da redundância

- Mais latência por envolver uma chamada de tool adicional.
- Maior consumo de contexto/tokens ao trafegar texto bruto e versão estruturada.
- Mais complexidade no fluxo sem aumento proporcional de qualidade na correção.

### Decisão adotada

Para casos comuns de erro de execução e falha de teste, o agente consome o log bruto diretamente.

A tool de parsing continua disponível para cenários específicos, por exemplo:
- normalização de logs de múltiplos formatos/fontes,
- extração determinística de campos para auditoria,
- integração com pipelines que exigem JSON estruturado.

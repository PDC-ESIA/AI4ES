...
Sua única entrega possível é um arquivo .mmd válido, persistido diretamente por você em staging.
...
⚠️ VERIFICAÇÃO DE PRÉ-REQUISITO: Sua primeira ação deve ser listar os arquivos disponíveis em staging diretamente.
...
3. Gere TODOS os diagramas do lote em uma única resposta, persistindo cada arquivo diretamente em staging sem aguardar confirmação entre eles.
...
Caso contrário, liste os arquivos .md disponíveis em staging diretamente.
...
Execute cada verificação antes de persistir o arquivo em staging.
...
Persista diretamente em staging: Doubt_Artifact_<hu_id>_<valor retornado pela ferramenta de data atual>.md
...
Após aprovação no PASSO 2, persista o arquivo diretamente em staging sem aguardar confirmação:

...
_ERROR_CLASSIFIER: list[tuple[re.Pattern, ErrorType, str]] = [
    ...
    (re.compile(r"não suportado", re.I),
     ...
     "Use um dos formatos suportados: 'mmd' ou 'md'."),
]
...
class ArtifactGatekeeper:
    ...
    Uso típico (chamado pelo Supervisor ou diretamente pelo Arquiteto):
        ...
        result = ArtifactGatekeeper.check(content="flowchart TD\\n  A --> B", format="mmd")
        if not result.valid:
    ...
    def check(content: str, format: str) -> GatekeeperResult:
        ...
        format  : str   Formato declarado:'mmd' | 'md'
        ...
        if format_clean not in ("mmd", "md"):
            return GatekeeperResult(
                valid=False,
                error_type=ErrorType.UNSUPPORTED_FORMAT,
                error_message=f"Formato '{format}' não suportado. Aceitos:'mmd', 'md'.",
...
if __name__ == "__main__":
    ...
    cases = [
        ...
        ("@startuml xxxx  @enduml",                                                  "puml",  "Formato não suportado")
    ]
